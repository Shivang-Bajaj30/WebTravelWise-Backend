...
def _generate_itinerary_chunk(destination, travelers, start_date, end_date, preferences):
  ...
  Guidelines:
  1. Suggest realistic activities, places, and experiences for the destination.
  2. Include timing (morning/afternoon/evening), pricing in INR, and best time to visit.
  3. Recommend hotels with realistic names, approximate prices, and amenities.
  4. Suggest local transportation methods (metro, cab, rental bike, etc.).
  5. Provide approximate cost breakdowns in INR (e.g. food, hotel, transport).
  6. The final JSON must strictly follow this schema (NO markdown, NO text outside JSON):
  ...
  try:
    ...
    return {
      "places": [
        {"name": "Burj Khalifa", "details": "Iconic Dubai landmark", "time": "Morning", "pricing": "₹2800", "bestTime": "Evening"}
      ],
      "hotels": [
        {"name": "Atlantis The Palm", "address": "Crescent Rd, The Palm Jumeirah, Dubai",
...
def generate_itinerary(destination, travelers, start_date, end_date, preferences):
  ...
  return _generate_itinerary_chunk(destination, travelers, start_date, end_date, preferences)
